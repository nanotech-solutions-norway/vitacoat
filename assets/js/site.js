document.addEventListener('DOMContentLoaded', () => {
  ...
  const uploadedImageMap = {
    'vitacoat-antimicrobial-hero.svg': 'frontpage-hero.jpg',
    'frontpage-hero.jpeg': 'frontpage-hero.jpg',
    'vitacoat-documentation-panel.svg': 'vitacoat-documentation-panel.jpg',
    'vitacoat-documentation-panel.jpeg': 'vitacoat-documentation-panel.jpg',
    'vitacoat-healthcare-application.svg': 'vitacoat-healthcare-application.jpg',
    'vitacoat-healthcare-application.jpeg': 'vitacoat-healthcare-application.jpg',
    'vitacoat-food-processing-application.svg': 'vitacoat-food-processing-application.jpg',
    'vitacoat-food-processing-application.jpeg': 'vitacoat-food-processing-application.jpg',
    'vitacoat-public-facilities-application.svg': 'vitacoat-public-facilities-application.jpg',
    'vitacoat-public-facilities-application.jpeg': 'vitacoat-public-facilities-application.jpg',
    'vitacoat-education-offices-application.svg': 'vitacoat-education-offices-application.jpg',
    'vitacoat-education-offices-application.jpeg': 'vitacoat-education-offices-application.jpg',
    'vitacoat-electronics-touchpoints-application.svg': 'vitacoat-electronics-touchpoints-application.jpg',
    'vitacoat-electronics-touchpoints-application.jpeg': 'vitacoat-electronics-touchpoints-application.jpg'
  };

  const routeHeroMap = {
    '/': 'frontpage-hero.jpg',
    '/en/': 'frontpage-hero.jpg',
    '/applications/healthcare/': 'vitacoat-healthcare-application.jpg',
    '/en/applications/healthcare/': 'vitacoat-healthcare-application.jpg',
    '/applications/food-processing/': 'vitacoat-food-processing-application.jpg',
    '/en/applications/food-processing/': 'vitacoat-food-processing-application.jpg',
    '/applications/public-facilities/': 'vitacoat-public-facilities-application.jpg',
    '/en/applications/public-facilities/': 'vitacoat-public-facilities-application.jpg',
    '/applications/education-and-offices/': 'vitacoat-education-offices-application.jpg',
    '/en/applications/education-and-offices/': 'vitacoat-education-offices-application.jpg',
    '/applications/electronics-and-touchpoints/': 'vitacoat-electronics-touchpoints-application.jpg',
    '/en/applications/electronics-and-touchpoints/': 'vitacoat-electronics-touchpoints-application.jpg',
    '/documentation/': 'vitacoat-documentation-panel.jpg',
    '/en/documentation/': 'vitacoat-documentation-panel.jpg'
  };
  ...
  const placeholderSvg = filename => `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900"><rect width="1600" height="900" fill="#f3f2ef"/><rect x="35" y="35" width="1530" height="830" rx="26" fill="none" stroke="#4950bc" stroke-width="8"/><text x="800" y="420" text-anchor="middle" font-family="Arial" font-size="48" font-weight="700" fill="#171717">${String(filename || 'missing-image').replace(/[<>&]/g, '')}</text><text x="800" y="505" text-anchor="middle" font-family="Arial" font-size="26" fill="#4c5060">Missing or not deployed in /assets/img/</text></svg>`)}`;

  const makeLanguageSwitch = (placement = '') => {
    const wrap = document.createElement('div');
    wrap.className = `language-switcher flag-language-switcher ${placement}`;
  ...
  const makeLogChart = () => {
    ...
    wrapper.setAttribute('aria-label', 'Horizontal bar chart showing log reduction values: EN 13727 bactericidal assessment 5, EN 13624 Candida 4.2, EN 14476 enveloped viruses 4, SARS-CoV-2 proxy 2.3.');
    const inner = document.createElement('div');
    inner.className = 'vc-log-chart-inner';
    ...
    chartLabels.forEach((label, idx) => {
      ...
      row.className = 'vc-chart-row';
      const y = document.createElement('div');
      y.className = 'vc-y-label';
      y.textContent = label;
      const track = document.createElement('div');
      track.className = 'vc-track';
      const bar = document.createElement('div');
      bar.className = 'vc-bar';
      bar.style.setProperty('--w', chartData[idx].width);
      bar.style.setProperty('--c', chartData[idx].color);
      bar.textContent = chartData[idx].value;
    ...
    ticks.className = 'vc-axis-ticks';
    chartTicks.forEach(t => { const span = document.createElement('span'); span.textContent = t; ticks.appendChild(span); });
    const axisTitle = document.createElement('div');
    axisTitle.className = 'vc-axis-title';
    axisTitle.textContent = 'Log Reduction';
    axis.appendChild(ticks);
    axis.appendChild(axisTitle);
    inner.appendChild(axis);
    wrapper.appendChild(inner);
    requestAnimationFrame(() => setTimeout(() => wrapper.querySelectorAll('.vc-bar').forEach(b => b.classList.add('is-animated')), 120));
    return wrapper;
  };
  ...
  const makePerformanceSection = () => {
    ...
    section.innerHTML = `<div class="container panel"><div class="eyebrow">${eyebrow}</div><h2 align="center">${heading}</h2><p align="center" class="vc-chart-explainer">${intro}</p><div class="vc-chart-copy"><div class="vc-chart-slot"></div><div class="callout">${caution}</div></div></div>`;
  ...
  const renderLogReductionChart = () => {
    ...
    const old = document.querySelector('.bar-group');
    if (old && old.closest('.section')) { old.closest('.section').replaceWith(section); return; }
    ...
    if (after) after.insertAdjacentElement('afterend', section); else if (main) main.appendChild(section);
  };

  const renderLogo = () => document.querySelectorAll('.brand').forEach(b => { b.textContent = ''; b.setAttribute('aria-label', 'VitaCoat homepage'); b.href = routes.home; });
  ...
  const renderFooter = () => document.querySelectorAll('.site-footer .footer-grid').forEach(f => {
    f.innerHTML = '';
